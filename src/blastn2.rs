...
impl HSP {
    fn try_join(
        &mut self,
        other: &mut HSP,
        scheme: &Arc<ScoringScheme>,
        params: &Arc<Params>,
        query: &Arc<SimpleRecord>,
        max_d: usize,
    ) -> bool {
        ...
        if other.is_joined
            || self.id != other.id
            || (self.idx_in_query > other.idx_in_query
                || self.idx_in_query + self.virtual_length
                    >= other.idx_in_query + other.virtual_length)
            || self.idx_in_query
                + self.word.len()
                + self.padding_right
                + max_d
                ...
                + other.padding_left
                < other.idx_in_query
            || self.idx_in_record + self.padding_left + self.word.len()
                > other.idx_in_record + other.padding_left
        {
            return false;
        }
        let d_in_rec = (self.idx_in_record + self.padding_left + self.word.len())
            .abs_diff(other.idx_in_record + other.padding_left);
        ...
        if d_in_rec >= d_in_q || self.idx_in_query + self.virtual_length < other.idx_in_query - 1 {
            if self
                .try_join_separate(other, scheme, params, query, max_d)
                .is_err()
            {
                return false;
            }
        } else if self.try_join_overlapping(other, scheme).is_err() {
            return false;
        }
        true
    }

    fn try_join_overlapping(
        &mut self,
        other: &mut HSP,
        scheme: &Arc<ScoringScheme>,
    ) -> Result<(), &str> {
        ...
        if self.idx_in_query + self.virtual_length > other.idx_in_query + other.virtual_length
            ...
            || self.idx_in_record + self.padding_left + 1 + self.word.len() < other.idx_in_record
        {
            //|| self.idx_in_query > other.idx_in_record {
            return Err("false match");
        }
        let start_idx: usize = if other.num_gaps > 0 {
            let start = (self.idx_in_query + self.virtual_length).abs_diff(other.idx_in_query);
            recursive_count_gaps(&self.word, start)
        } else {
            (self.idx_in_query + self.virtual_length).abs_diff(other.idx_in_query)
        };

        let other_overlap_gaps = count_chars(&other.word[..start_idx], b'-');
        self.score += get_score(
            &other.word[start_idx..],
            &other.db_seq[other.padding_left + start_idx..other.padding_left + other.word.len()],
            scheme,
        );
        ...
        if other.idx_in_record + other.db_seq.len() > self.idx_in_record + self.db_seq.len() {
            self.db_seq.extend_from_slice(
                &other.db_seq[self.db_seq.len() - (other.idx_in_record - self.idx_in_record)..],
            );
            self.padding_right = self.db_seq.len() - self.word.len() - self.padding_left;
        } else {
            self.padding_right = self.db_seq.len() - self.word.len() - self.padding_left;
    ...
    fn try_join_separate(
        ...
        let d_in_q = (self.idx_in_query + self.virtual_length).abs_diff(other.idx_in_query);
        let d_in_rec = (self.idx_in_record + self.word.len() + self.padding_left)
            .abs_diff(other.idx_in_record + other.padding_left);
        ...
        if self.idx_in_query + self.virtual_length >= other.idx_in_query {
            ...
            if *self
                .word
                .last()
                .expect("could not get last element in word")
                == b'-'
            {
            ...
            self.word.append(&mut vec![b'-'; max_gaps]);
            ...
            self.word.extend_from_slice(
                &query.seq[self.idx_in_query + self.virtual_length
                    ..self.idx_in_query + self.virtual_length + d_in_q],
            );
            self.word.append(&mut vec![b'-'; max_gaps]);
            self.num_gaps += max_gaps;
            ...
            self.set_vlen();
            self.score += get_score(
                &self.word[self.word.len() - d_in_q..],
                &self.db_seq[self.padding_left + self.word.len() - d_in_q
                    ..self.padding_left + self.word.len()],
                scheme,
            );
    ...
    fn extend_right_ungapped(
        ...
        self.set_vlen();
        let mut best_ext = 0;
        for _i in 1..max_extension
            .min(query.seq.len() - self.idx_in_query - self.virtual_length)
            .min(self.padding_right + 1)
        {
            best_ext += 1;
    ...
    fn _extend_left(
        ...
        for i in 0..max_extension {
            if self.idx_in_query == 0
                || self.padding_left == 0
                || self.score < params.extension_threshold
            {
                break;
            }
    ...
    fn _extend_right(
        ...
        for i in 0..max_extension {
            if self.padding_right == 0
                || self.idx_in_query + self.virtual_length + 1 >= query.seq.len()
                || self.score < params.extension_threshold
            {
                break;
            }
        ...
        let _ = self
            .word
            .split_off(self.word.len() - (extended - best_extension));
    ...
    fn _extend_one_right_unchecked(
        &mut self,
        scheme: &Arc<ScoringScheme>,
        query: &Arc<SimpleRecord>,
        can_use_gaps: bool,
        ...
        if query.seq[self.idx_in_query + self.virtual_length + 1] == b'N' {
            self.word.push(b'N');
        ...
        if self.db_seq[self.padding_left + self.word.len()]
            == query.seq[self.idx_in_query + self.virtual_length]
        {
            self.score += scheme.hit;
            self.word
                .push(query.seq[self.idx_in_query + self.virtual_length]);
            self.padding_right -= 1;
            self.virtual_length += 1;
            false
        } else {
            if !can_use_gaps
                || (self.padding_right < 2
                    || (self.idx_in_query - self.virtual_length) < query.seq.len() - 2)
                || query.seq[self.idx_in_query + self.virtual_length + 1]
                    == self.db_seq[self.padding_left + self.word.len() + 1]
            {
                self.word
                    .push(query.seq[self.idx_in_query + self.virtual_length + 1]);
                self.score += scheme.miss;
                self.padding_right -= 1;
                self.virtual_length += 1;
                return false;
            ...
            self.word.push(b'-');
            if self.word[self.word.len() - 2] == b'-' {
    ...
    fn _extend_one_left_unchecked(
        &mut self,
        scheme: &Arc<ScoringScheme>,
        query: &Arc<SimpleRecord>,
        can_use_gaps: bool,
    ) -> bool {
        if query.seq[self.idx_in_query - 1] == b'N' {
            self.word.insert(0, b'N');
            // save the information of a masked region, as that influences stats.
            self.padding_left -= 1;
            self.idx_in_query -= 1;
            self.virtual_length += 1;
        ...
        if self.db_seq[self.padding_left - 1] == query.seq[self.idx_in_query - 1] {
            ...
            if !can_use_gaps
                || (self.padding_left < 2 || self.idx_in_query < 2)
                || query.seq[self.idx_in_query - 2] == self.db_seq[self.padding_left - 2]
            {
                self.word.insert(0, query.seq[self.idx_in_query - 1]);
                self.score += scheme.miss;
                self.padding_left -= 1;
    ...
    fn contains(&self, other: &HSP) -> bool {
        self.idx_in_query <= other.idx_in_query
            && self.idx_in_query + self.virtual_length >= other.idx_in_query + other.virtual_length
            && self.idx_in_record + self.padding_left <= other.idx_in_record + other.padding_left
            && self.idx_in_record + self.padding_left + self.word.len()
                >= other.idx_in_record + other.padding_left + other.word.len()
    }
}
...
impl fmt::Display for HSP {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        writeln!(
            f,
            "start_idx rec: {}",
            self.idx_in_record + self.padding_left
        )?;
        writeln!(
            f,
            "v length: {}, gaps: {}, length: {}",
            self.virtual_length,
            self.num_gaps,
            self.word.len()
        )?;
        ...
        for j in (0..self.word.len().min(max_chars)).step_by(chars_per_line) {
            ...
            writeln!(f)?;
            write!(f, "DB:    ")?;
            for b in &self.db_seq[self.padding_left + j
                ..(self.padding_left + self.word.len()).min(j + self.padding_left + chars_per_line)]
            {
                write!(f, "{}", convert_to_ascii(b))?;
            }
            writeln!(f)?;
...
impl Debug for HSP {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        writeln!(
            f,
            "\nquery info: idx {}, len {}, end {}",
            self.idx_in_query,
            self.word.len(),
            self.idx_in_query + self.word.len()
        )?;
        writeln!(
            f,
            "db info   : idx {}, len {}, end {}, pad l {} r {}, tot {}\n",
            self.idx_in_record,
            self.db_seq.len(),
            self.idx_in_record + self.db_seq.len(),
            self.padding_left,
            self.padding_right,
            self.padding_left + self.word.len() + self.padding_right
        )?;
        Ok(())
    }
}
...
fn recursive_count_gaps(seq: &[u8], idx: usize) -> usize {
    let gaps = count_chars(&seq[..idx], b'-');
    if gaps == 0 {
        idx
    } else {
        idx + recursive_count_gaps(&seq[idx..], gaps)
    }
}
...
pub fn align(
    ...
    let lengths = records
        .by_ref()
        .map(|r| {
            (r.end_byte as usize - 1) - (r.start_byte as usize - 1) + (4 - r.start_bit) + r.end_bit
        })
        .collect::<Vec<usize>>();
    records.reset();
    let info = Info {
...
fn process_hits(
    ...
    let mut all_hits = all_hits
        .into_iter()
        .flatten()
        .collect::<Vec<Rc<RefCell<HSP>>>>();
    ...
    for h in all_hits.iter() {
        ...
        if !h.borrow().is_extended_left {
            ...
            h.borrow_mut()
                .extend_left_ungapped(&scheme, &query, &params, 100);
        }
        if !h.borrow().is_extended_right {
            //h.borrow_mut().extend_right(&scheme, &query, &params, 100, 500);
            h.borrow_mut()
                .extend_right_ungapped(&scheme, &query, &params, 100);
        }
    }
    let _ = all_hits
        .iter()
        .map(|h| {
            ...
            let e_val = ((h.borrow().word.len() - count_chars(&h.borrow().word, b'N'))
                * infos.get_length_of(&h.borrow().id)) as f64
                * 2.0_f64.powf(-bit_s);
    ...
    println!(
        "{} hits found, {} non joined hits",
        h,
        all_hits
            .iter()
            .map(|i| if i.borrow().is_joined { 0 } else { 1 })
            .sum::<i32>()
    );
    println!("\nBest hit: ");
    print!(
        ...
        all_hits
            .last()
            .unwrap_or(&Rc::new(RefCell::new(HSP::default())))
            .borrow()
    );
    if params.verbose {
        ...
        h.set_vlen();
        compare_to_query(
            &query.seq[h.idx_in_query..h.idx_in_query + h.virtual_length],
            &h.word,
        );
    }
}
...
fn join_left(
    ...
    let h: Rc<RefCell<HSP>> = if hits[hits.len() - 1].borrow_mut().try_join(
        &mut subject.borrow_mut(),
        scheme,
        params,
        query,
        max_distance,
    ) {
        Rc::clone(&hits[hits.len() - 1])
    } else {
        Rc::clone(&subject)
    };
    join_left(
...
fn join_right(
    ...
    subject.borrow_mut().try_join(
        &mut hits[0].borrow_mut(),
        scheme,
        params,
        query,
        max_distance,
...
fn distribute_chunks(
    ...
    distributors: Vec<mpsc::Sender<ProcessedChunk>>,
    params: Arc<Params>,
    mut records: impl Iterator<Item = Arc<Record>>,
) {
    let mut i = 0;
    ...
    while not_finished {
        ...
        let chunk = process_chunk(
            ...
            &mut overlap_buf,
            bytes,
            &mut bytes_in_current_record,
        );
        distributors[i % distributors.len()]
            .send(chunk)
            .unwrap_or_else(|_| {
                panic!(
                    "failed to send chunk {} to worker thread {}",
                    i,
                    i % distributors.len()
                )
            });
...
fn process_chunk(
    seq: Vec<u8>,
    records: &mut impl Iterator<Item = Arc<Record>>,
    current_record: &mut Arc<Record>,
    total_bytes: &u128,
    params: &Params,
    overlap_buf: &mut Vec<u8>,
    received: usize,
    bytes_in_rec: &mut u128,
) -> ProcessedChunk {
    ...
    if overlap_buf.len() != params.k.div_ceil(4) + 2 * params.extension_length.div_ceil(4)
        || overlap_buf.is_empty()
        || seq.len() == overlap_buf.len()
    {
    ...
    if current_record.end_byte < *total_bytes {
        ...
        *overlap_buf = seq
            [seq.len() - params.extension_length.div_ceil(4) * 2 - params.k.div_ceil(4)..]
            .to_vec();
        chunk = ProcessedChunk {
...
fn scan(
    ...
    query: Arc<SimpleRecord>,
    scheme: Arc<ScoringScheme>,
    hit_sender: &mpsc::Sender<Vec<HSP>>,
) -> Result<(), String> {
    let mut hits = Vec::default();
    for (i, slice1) in chunk.bytes[chunk.start_byte..=chunk.end_byte]
        .windows(params.k.div_ceil(4))
        .enumerate()
    {
        for (j, slice2) in query.words.iter().enumerate() {
            ...
            let seq = bytes_to_chars(
                &chunk.bytes[0.max(
                    i + chunk.start_byte
                        - (params.extension_length.div_ceil(4)).min(i + chunk.start_byte),
                )
                    ..chunk.bytes.len().min(
                        i + params.extension_length.div_ceil(4)
                            + params.k.div_ceil(4)
                            + chunk.start_byte,
                    )],
                if chunk.bytes.len().min(
                    i + params.extension_length.div_ceil(4)
                        + params.k.div_ceil(4)
                        + chunk.start_byte,
                ) == chunk.bytes.len()
            ...
            hits.push(HSP {
                ...
                padding_right: params
                    .extension_length
                    .min((chunk.bytes.len() - i - params.k.div_ceil(4)) * 4 - 3 + chunk.end_bit),
                db_seq: seq,
                word: bytes_to_chars(&query.words[j], 3, 0),
                idx_in_query: j,
                idx_in_record: chunk.start_in_rec as usize + i * 4 + chunk.start_byte * 4
                    - params.extension_length.min((chunk.start_byte + i) * 4),
...
fn _get_byte_score(s1: &[u8], s2: &[u8], scheme: &Arc<ScoringScheme>) -> i16 {
    (0..s1.len())
        .map(|i| _get_bitwise_score(&s1[i], &s2[i], scheme, 4))
        .sum()
}
...
fn get_query_words(k: usize, query: &mut SimpleRecord) {
    ...
    query.words = (0..query.seq.len() - k + 1)
        .map(|i| {
            if !query.seq[i..i + k].contains(&b'N') {
